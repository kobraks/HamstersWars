...
namespace game
{
	bool Keyboard::is_pressed(const int& key)
	{
		if (is_legal(key))
	...
	bool Keyboard::is_up(const int& key)
	{
		if (is_legal(key))
	...
	void Keyboard::add_action_on_key_released(const key_up_action& on_release)
	{
		get_instance()->on_key_released_ += on_release;
	}

	void Keyboard::add_action_on_key_pressed(const key_pressed_action& on_pressed)
	{
	...
	void Keyboard::parse_event(const sf::Event& event)
	{
		switch (event.type)
		{
	...
	void Keyboard::clear_pressed_keys()
	{
	...
	bool Keyboard::is_legal(const int& key)
	{
	...
	void Keyboard::register_class(LuaIntf::LuaBinding& binding) const
	{
		using namespace sf;
